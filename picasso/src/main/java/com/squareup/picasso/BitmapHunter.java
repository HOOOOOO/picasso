...
class BitmapHunter implements Runnable {
	 ...
	 * well as potential OOMs. Shamelessly stolen from Volley.(从Volley学来的)
	 */
	...
	/**
	 * 获取图片 先是判断是否应该从内存缓存中查找 如果在内存缓存中找到 则直接返回 如果在内存中找不到 则进行
	 * 网络请求 获取到图片之后 会判断是否进行变换 而且每次只变换一张图片 防止OOM
	 *
	 * @return bitmap
	 * @throws IOException
	 */
	Bitmap hunt() throws IOException {
		...
		if (bitmap != null) {
			...
			if (data.needsTransformation() || exifOrientation != 0) {
				// 使用this表示使用当前对象作为锁，this并不是所有的地方都能使用，例如静态方法里面，当然这
				// 时候加的所可能就不是普通对象锁而是类对象锁。
				// 使用自定义对象作为锁，一般是在没有明确对象作为锁得前提下使用这种方式，但是直接new一个
				// object对象作为锁，代价高，可以创建一个零长度的byte数组作为锁代价是最低的。
				// byte[] lock = new byte[0];
				// synchronized(lock){
				// 	 ....
				// }
				// 0长度字节数组创建起来比任何其他对象创建都经济，生成0长度字节数组对象只需要3行操作码，
				// new Object()需要7行操作码。
				//
				// 这里指定了用哪个对象的钥匙
